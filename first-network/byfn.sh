...
export CA_FILE="docker-compose-e2e.yaml"
...
function clearContainers() {
  CONTAINER_IDS=$(docker ps -a | awk '($2 ~ /dev-peer.*.passport.*/) {print $1}')
...
function removeUnwantedImages() {
  DOCKER_IMAGE_IDS=$(docker images | awk '($1 ~ /dev-peer.*.passport.*/) {print $3}')
...
# Versions of fabric known not to work with this release of nip-network
...
function networkUp() {
  ...
  if [ "${IF_COUCHDB}" == "couchdb" ]; then
    if [ "$CONSENSUS_TYPE" == "kafka" ]; then
      IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_KAFKA -f $COMPOSE_FILE_COUCH -f $CA_FILE up -d 2>&1
    else
      IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_COUCH -f $CA_FILE up -d 2>&1
    fi
  else
    if [ "$CONSENSUS_TYPE" == "kafka" ]; then
      IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_KAFKA -f $CA_FILE -f $COMPOSE_FILE_COUCH up -d 2>&1
    else
      IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE -f $CA_FILE -f $COMPOSE_FILE_COUCH up -d 2>&1
    fi
  fi
  if [ $? -ne 0 ]; then
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run -v $PWD:/tmp/nip-network --rm hyperledger/fabric-tools:$IMAGETAG rm -Rf /tmp/nip-network/ledgers-backup
